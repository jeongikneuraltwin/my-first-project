...
- 테넌트 단위 3D 디지털 트윈 요청 현황 확인  
  - 해당 테넌트에 대해 생성된 3D 제작 요청(프로젝트) 목록 조회  
  - 상세 보기를 통해 **3.5 3D Digital Twin Production** 화면으로 이동하여 진행 상태 관리  
- 데이터 재집계 관련 액션  
  - 해당 테넌트에 대해 재집계 Job 실행 이력 조회  
  - 필요 시 **3.3 Data Pipeline & Quality** 또는 **3.8 Admin Tools & Settings**에서  
...
- 테넌트별/전체 **사용량을 모니터링**하고, 기능·플랜 설계 및 향후 과금 정책 수립에 참고할 수 있는 데이터를 제공한다.  
- 현 단계에서는 **한도 설정·강제 차단 등 제한 기능은 제공하지 않고**,  
  사용량 추세를 파악하기 위한 **관찰·리포트 용도**에 초점을 둔다.
...
> 주석:  
> `license_management` 등에 저장된 플랜/라이선스 정보는  
> **현 시점에서는 참고용 메타데이터**로만 사용하며,  
> 실제 사용량을 제한하거나 차단하는 로직에는 연결하지 않는다.

#### 3.2.3 관리 액션

- 특정 테넌트의 사용량 상세 조회  
  - 기간(예: 최근 7일, 최근 30일 등) 기준 필터링  
  - API 호출 유형별, 기능별(시뮬레이션 타입 등) 사용량 분포 확인  
- 사용량 리포트 다운로드  
  - 전역/테넌트별 사용량 CSV 또는 PDF 내보내기  
  - 영업/CS/기획팀이 플랜 설계 및 업셀링 논의 시 참고 자료로 활용  
- 이상 패턴 마킹(메모 수준)  
  - 특정 테넌트의 사용량이 비정상적으로 급등/급감한 경우  
  - 내부 메모/태그로 표시하여 추후 분석 및 정책 설계 시 참고  
...
1. **사용자 상황**  
   ...
   - 테넌트 앱에서 3D 디지털 트윈 제작 요청을 생성하면,  
     관리자 콘솔에서는 상태가 `Requested` 인 3D 프로젝트로 기록된다.
